...
export function BorderDefenseWorkspace({ mode, variant = "desktop", embedded = false }: { mode: BorderDefenseMode; variant?: "desktop" | "mobile"; embedded?: boolean }) {
  ...
  return <main className="flex h-[100dvh] w-full overflow-hidden bg-[#0B1620] text-[#EAEAEA]">
    ...
    <section className="flex min-w-0 flex-1 flex-col">
      ...
      {mode === "daily" ? <DailyReportForm key={chat.activeTaskId ?? "new-task"} loading={chat.loading} onSubmit={(date, reportType) => chat.submit({ date, reportType })} onCancel={chat.stopTask} /> : <div className="p-3"><div className="mx-auto flex max-w-4xl items-end gap-2 rounded-xl border border-[#31506A] bg-[#172B3A] p-2"><textarea value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); send(); } }} placeholder="请输入边防数据问题…" rows={2} className="max-h-36 min-h-12 flex-1 resize-none bg-transparent px-2 py-1 text-sm outline-none placeholder:text-[#617A8D]" />{chat.loading ? <button onClick={chat.stopTask} title="停止接收" className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#A94141]"><Square className="h-4 w-4" /></button> : <button onClick={send} disabled={!query.trim()} className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#00AFC8] disabled:opacity-40"><Send className="h-4 w-4" /></button>}</div></div>}
    </section>
  </main>;
}
