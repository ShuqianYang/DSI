...
export function TaskSidebar({ tasks, activeId, onSelect, onRemove, onNew, mobile, onClose }: { tasks: BorderTaskItem[]; activeId?: string; onSelect: (task: BorderTaskItem) => void; onRemove: (id: string) => void; onNew: () => void; mobile?: boolean; onClose?: () => void }) {
  ...
  return <aside aria-label="历史任务" className={`${mobile ? "fixed inset-y-0 left-0 z-50 w-[82vw] max-w-[320px] shadow-2xl" : "relative w-[280px]"} flex h-full shrink-0 flex-col border-r border-[#263849] bg-[#101D2A]`}>
    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-3">{tasks.length === 0 && <div className="p-6 text-center text-xs text-[#71869A]">暂无历史任务</div>}{groups.map((group) => <section key={group.key} className="mb-3"><div className="sticky top-0 z-10 bg-[#101D2A]/95 px-2 py-1.5 text-[11px] font-medium text-[#6F8CA2] backdrop-blur">{group.label}</div><div className="space-y-1">{group.tasks.map((task) => <button key={task.id} onClick={() => { onSelect(task); onClose?.(); }} className={`group flex w-full items-start gap-2 rounded-md p-2.5 text-left ${activeId === task.id ? "bg-[#083D52]" : "hover:bg-[#172A3A]"}`}><span className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${task.status === "failed" ? "bg-[#FF4444]" : task.status === "completed" ? "bg-[#44FF44]" : "animate-pulse bg-[#00E0FF]"}`} /><span className="min-w-0 flex-1"><span className="line-clamp-2 text-xs leading-4 text-[#EAEAEA]">{task.title}</span><span className="mt-1 block text-[10px] text-[#71869A]">创建 {new Date(task.createdAt).toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit", hour12: false })} · #{task.id.slice(0, 8)} · {task.status === "failed" ? "失败" : task.status === "completed" ? "已完成" : "执行中"}</span>{task.completedAt && <span className="mt-0.5 block text-[10px] text-[#60788B]">完成 {new Date(task.completedAt).toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit", hour12: false })}</span>}</span><span onClick={(event) => { event.stopPropagation(); onRemove(task.id); }} className="hidden p-1 text-[#71869A] hover:text-[#FF6666] group-hover:block"><Trash2 className="h-3.5 w-3.5" /></span></button>)}</div></section>)}</div>
