...
// Test 1: generateChartPng for bar chart
{
  ...
  assert.ok(buffer.length > 1000, "PNG buffer should contain a rendered chart");
  assert.equal(buffer[0], 0x89, "PNG buffer should start with PNG magic bytes");
  const metadata = await sharp(buffer).metadata();
  assert.equal(metadata.format, "png", "chart should be a decodable PNG");
  assert.equal(metadata.width, 800, "chart should use the expected width");
  assert.equal(metadata.height, 480, "chart should use the expected height");
  const raw = await sharp(buffer).raw().toBuffer();
  assert.deepEqual([...raw.subarray(0, 3)], [255, 255, 255], "chart background should be white");
